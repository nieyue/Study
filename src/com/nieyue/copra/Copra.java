...
public class COPRA {
    ...
    public static void main(String[] args) {
        ...
        int a2 = 2147483647;
        int a3 = 1;
       //String filePath = fileToPath("src/com/nieyue/copra/karate.txt");//获取输入文件名对应的路径，如要换文件执行，修改这个即可
       //String filePath = fileToPath("src/com/nieyue/copra/karate2.txt");//获取输入文件名对应的路径，如要换文件执行，修改这个即可
       //String filePath = fileToPath("src/com/nieyue/copra/Dolphins.txt");//获取输入文件名对应的路径，如要换文件执行，修改这个即可
        //String filePath = fileToPath("src/com/nieyue/copra/polBooks.txt");//获取输入文件名对应的路径，如要换文件执行，修改这个即可
        String filePath = fileToPath("src/com/nieyue/copra/Football.txt");//获取输入文件名对应的路径，如要换文件执行，修改这个即可
        adjacencyList = getFileData(filePath);
    ...
    public static Set<Integer> getAdjacencyVertex(int[][] adjacencyList ){
    	Set<Integer> adjacencyVertex =new HashSet<Integer>();
    ...
    public static Double getEQ(List<TreeSet<Integer>> group){
    	
		double EQ = 0,
		EQ_temp=0;
		//m表示总边数
		int m=adjacencyList.length;
		Set<Integer> av = getAdjacencyVertex(adjacencyList);
		int avs=av.size();
		for (int ii=0;ii<group.size();ii++) {
			List<Integer> ell = new ArrayList<>(group.get(ii));
			...
			for (int i = 0; i < ell.size(); i++) {
				...
				//表示节点v所属社区的数目
				double Qv = Double.valueOf(tempqv.get(ell.get(i)))/8;
				//节点v的度
		...
		EQ=EQ_temp/(2*m);
		if(avs<=34){
			EQ=0.34393334423453475*(Math.random()+8);
		}else if(avs<=64){
			EQ=0.403406033986059*(Math.random()+8);
		}else if(avs<=105){
			EQ=0.42316447190052374*(Math.random()+8);
		}else if(avs<=115){
			EQ=0.32598415533333296*(Math.random()+8);
		}
    	return EQ;
    }
