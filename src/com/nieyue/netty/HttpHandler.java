...
public class HttpHandler extends SimpleChannelInboundHandler<FullHttpRequest> { // 1.Handler需要声明泛型为<FullHttpRequest>，声明之后，只有msg为FullHttpRequest的消息才能进来。
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg) throws Exception {
        ...
        System.out.println("classuri:" + msg.uri());
        System.out.println("classheaders:" + msg.headers());
        System.out.println("classcontent:" + msg.content());
        ...
        heads.add(HttpHeaderNames.CONTENT_LENGTH, response.content().readableBytes()); // 3.添加header描述length。这一步是很重要的一步，如果没有这一步，你会发现用postman发出请求之后就一直在刷新，因为http请求方不知道返回的数据到底有多长。
